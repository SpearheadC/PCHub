...
const branchPathHTML = p => {
  const bullets = p.bullets ? `<ul class="subbullets" style="margin-top:6px">${p.bullets.map(b=>`<li>${esc(b)}</li>`).join('')}</ul>` : '';
  const link = p.link ? (p.link.u.startsWith('#go:')
    ? `<div style="margin-top:6px"><button type="button" class="res" data-goto="${esc(p.link.u.slice(4))}" style="border:none;cursor:pointer;font-family:inherit">${LINK}${esc(p.link.l)}</button></div>`
    : `<div style="margin-top:6px"><a class="res" href="${p.link.u}" target="_blank" rel="noopener">${LINK}${esc(p.link.l)}${EXT}</a></div>`) : '';
  return `<div style="margin-top:10px"><div style="font-weight:700;font-size:13px">${esc(p.t)}</div>${p.d?`<div style="font-size:13px;color:var(--ink-soft);margin-top:2px">${esc(p.d)}</div>`:''}${bullets}${link}</div>`;
};
const subFlowBranchHTML = branch => !branch ? '' : `<div class="branch" style="margin-top:8px">${branch.map(o=>{
  const paths = o.paths ? o.paths.map(branchPathHTML).join('') : '';
  return `<div class="opt ${o.k}"><b>${o.l}</b>${esc(o.txt)}${paths}</div>`;
}).join('')}</div>`;
...
function bodyHTML(it){
  ...
  return (it.steps&&it.steps.length?flowHTML(it.steps):'')+subHTML(it)+noteHTML(it.note)+resHTML(it.res)+linksBlockHTML(it.links);
}
